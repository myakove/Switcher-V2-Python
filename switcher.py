...
switcherIP = "192.168.10.10"
phone_id = "a71e"
device_id = "09c01e"
device_pass = "34363733"
...
""")
	print("Usage:\r")
	print("======\r")
	print(f"Off command: ./{APP_NAME} 0\r")
	print(f"On command: ./{APP_NAME} 1\r")
	print(f"On command duration in minutes: ./{APP_NAME} t30\r")
	print(f"Get State: ./{APP_NAME} 2\r")
	print(f"Set Auto shutdown setting (in hours): ./{APP_NAME} m03:00\r")
	print(f"Retrieve Schedule from device: ./{APP_NAME} list\r")
	print(f"Create a Schedule: ./{APP_NAME} create\r")
	print(f"Delete Schedule from device: ./{APP_NAME} del\r")
	print(f"Enable Schedule: ./{APP_NAME} enable\r")
	print(f"Disable Schedule: ./{APP_NAME} disable\r")
	print(f"Change switcher name: ./{APP_NAME} nNAME\r")
	print(f"Auto detect Switcher IP Address and state:./{APP_NAME} discover\r")
	print(f"Configure Switcher in AP Mode: ./{APP_NAME} configure\r")
	print(
		"Auto Extract the needed values for this script (device_id, phone_id and device_pass): "
		f"./{APP_NAME} extract\r\n"
	)
	sys.exit(1)
...
def crc_sign_full_packet_com_key(p_data, p_key):
	crc = ba.hexlify(struct.pack(">I", ba.crc_hqx(ba.unhexlify(p_data), 0x1021))).decode(
		"utf-8"
	)
	p_data = p_data + crc[6:8] + crc[4:6]
	crc = crc[6:8] + crc[4:6] + ba.hexlify(p_key).decode("utf-8")
...
def s_timer(s_minutes):
	s_seconds = int(s_minutes) * 60
	s_delay = struct.pack("<I", s_seconds)
	return ba.hexlify(s_delay).decode('utf-8')


# Get Power consumption and Elctrical current
def get_power(res):
	bin_out = ba.hexlify(res)[154:162]
	bin_int = int(bin_out[2:4] + bin_out[0:2], 16)
	return f"[+] Electric Current is: {(bin_int / float(220))}(A)\r\n[+] Power consumption is: {str(bin_int)}(W)"
...
def set_auto_close(hours):
	...
	if mSeconds < 3600:
		...
		return ba.hexlify(struct.pack("<I", mSeconds)).decode('utf-8')


def get_auto_close(res):
	...
	if len(split) == 0:
		...
		for i in range(len(split)):
			...
			if split[i][4:6] == "00":
				...
				print(
					"[+] Days: " + getDays(bytearray(ba.unhexlify((split[i][4:6])))[0])
				)
...
if sys.argv[1] == "extract":
	...
	try:
		...
		data = crc_sign_full_packet_com_key(data, pKey)
		print("[*] Sending Login Packet to Switcher...")
		s.send(ba.unhexlify(data))
		res = s.recv(1024)
		p_session2 = ba.hexlify(res)[16:24]
		if not p_session2:
			s.close()
			print(
			...
			s.send(ba.unhexlify(data))
			res = s.recv(1024)
			print("[+] Received SessionID: " + p_session2)
			print("\r\nInstructions:\r")
			print(
			...
			if len(res) != 87:
				...
				for i in range(0, 9999):
					...
					sys.stdout.write(
						"\r[*] Brute forcing Device Password, Please wait:"
						+ device_pass
					)
					sys.stdout.flush()
					data = (
					...
					if len(res) > 44 and len(res) < 60:
						...
						total_time = time.strftime(
							"%H:%M:%S", time.gmtime(brute_end - brute_start)
						)
...
if sys.argv[1] == "discover":
	try:
		...
		while True:
			print(
				"[*] Waiting for broadcast from Switcher device...(Press CTRL+C to abort)"
			)
			data, addr = sock.recvfrom(1024)
			if ba.hexlify(data)[0:4].decode('utf-8') != "fef0" and len(data) != 165:
				print("[!] Not a switcher broadcast message!")
			else:
				b = ba.hexlify(data)[152:160].decode('utf-8')
				ip_addr = int(b[6:8] + b[4:6] + b[2:4] + b[0:2], 16)
				print(
					"[+] Switcher IP Address: "
					+ socket.inet_ntoa(struct.pack("<L", ip_addr))
				)
				print("[+] Switcher MAC Address: " + ba.hexlify(data)[160:172].decode('utf-8').upper())
				print("[+] Switcher Name: " + data[42:74].decode('utf-8'))
				print("[+] Device ID: " + ba.hexlify(data)[36:42].decode('utf-8'))
				if ba.hexlify(data)[266:270].decode('utf-8') == "0000":
					print("[+] Device is OFF")
				else:
					print("[+] Device is ON")
				b = ba.hexlify(data)[310:318].decode('utf-8')
				open_time = int(b[6:8] + b[4:6] + b[2:4] + b[0:2], 16)
				imin = open_time / 60 % 60
				ihour = open_time / 60 / 60
				isec = open_time - ((imin * 60) + (ihour * 3600))
				print(
					"[*] Device is set to auto shutdown in: "
					+ (
						str(ihour)
						+ " Hour(s) "
						+ str(imin)
				...
				print(
					"[+] Electric Current is: %.1f" % (i / float(220))
					+ "(A)\r\n"
...
if sys.argv[1] == "configure":
	...
	while True and not completeFlag:
		...
		if (
			((not message and not completeFlag))
			or ((len(message)) != 8)
			or not (message.isdigit())
		):
	...
	message = input(
		"If your switcher is in access point mode press enter to continue..."
	)
	...
	try:
		...
		while True:
			data, addr = sock.recvfrom(1024)
			if (
				((ba.hexlify(data)[0:4] != "fef0" and len(data) != 165))
				or ((addr[0] != "192.168.1.1"))
				or ((addr[0] != "192.168.4.1"))
			):
	...
	try:
		...
		s.connect((switcherIP, 9957))
		data = (
			"fef0b1000232a20000000000340001000000"
			+ device_id
			+ get_ts().decode("utf-8")
			+ "00000000000000000000f0fe"
			+ phone_id
...
try:
	...
	s.send(ba.unhexlify(data))
	res = s.recv(1024)
	p_session2 = ba.hexlify(res)[16:24].decode('utf-8')
	if not p_session2:
		s.close()
		print("[!] Operation failed, Could not acquire SessionID, Please try again...")
		sys.exit()
	else:
		print(f"[+] Received SessionID: {p_session2}")
	...
	s.send(ba.unhexlify(data))
	res = s.recv(1024)
	print(f"[+] Device Name: {res[40:60].decode('utf-8')}")
	...
	if sys.argv[1] == "0" and state == "0000":
		...
		if not hourRe.match(sys.argv[1][1:]):
			...
			auto_close = set_auto_close(sys.argv[1][1:])
			data = (
				"fef05b0002320102"
				+ p_session2
				+ "340001000000000000000000"
		...
		data = (
			...
			+ "0000"
			+ device_pass
			+ "00000000000000000000000000000000000000000000000000000000060000"
		)
		data = crc_sign_full_packet_com_key(data, pKey)
		s.send(ba.unhexlify(data))
		res = s.recv(1024)
			...
			if not message and not completeFlag:
				...
				if message not in id_list:
					...
					data = (
						...
						+ "0000"
						+ device_pass
						+ "00000000000000000000000000000000000000000000000000000000080100"
						+ sch_id
					)
					data = crc_sign_full_packet_com_key(data, pKey)
					s.send(ba.unhexlify(data))
		...
		while True and not completeFlag:
			...
			if not start_time or not hourRe.match(start_time):
				...
				start_time = time.mktime(
					time.strptime(
						time.strftime("%d/%m/%Y") + " " + start_time, "%d/%m/%Y %H:%M"
					)
				)
		...
		while True and not completeFlag:
			...
			if not end_time or not hourRe.match(end_time):
				...
				end_time = time.mktime(
					time.strptime(
						time.strftime("%d/%m/%Y") + " " + end_time, "%d/%m/%Y %H:%M"
					)
				)
		...
		data = (
			...
			+ "0000"
			+ device_pass
			+ "00000000000000000000000000000000000000000000000000000000030c00ff"
			+ on_off
			+ week
			+ timstate
			+ start_time
		...
		if len(sys.argv[1][1:]) > 32 or len(sys.argv[1][1:]) < 1:
			...
			print(
				"[*] Changing device name from "
				+ res[40:72]
				+ " to: "
				+ sys.argv[1][1:]
			)
			switcher_name = (
				ba.hexlify(sys.argv[1][1:]) + (32 - len(sys.argv[1][1:])) * "00"
			)
			data = (
				"fef0740002320202"
				+ p_session2
		...
		while True and not completeFlag:
			message = input(
				'Enter The Schedule ID you want to enable (type "exit" to exit): '
			)
			if message == "exit":
				print("[+] Done!")
				break
				...
				if message not in id_list:
					...
					if on_off == "01":
						...
						data = (
							...
							+ "0000"
							+ device_pass
							+ "00000000000000000000000000000000000000000000000000000000070c00"
							+ time_id
							+ on_off
							+ week
							+ timstate
		...
		while True and not completeFlag:
			message = input(
				'Enter The Schedule ID you want to disable (type "exit" to exit): '
			)
			if message == "exit":
				print("[+] Done!")
				break
